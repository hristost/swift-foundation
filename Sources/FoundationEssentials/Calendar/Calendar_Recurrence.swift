        ...
        struct Iterator: Sendable, IteratorProtocol {
            ...
            internal init(start: Date, 
                ...
                let components: Calendar.ComponentSet = switch recurrence.frequency {
                    case .minutely: [.nanosecond, .second]
                    case .hourly:   [.nanosecond, .second, .minute]
                    case .daily:    [.nanosecond, .second, .minute, .hour]
                    case .weekly:   [.nanosecond, .second, .minute, .hour, .weekday]
                    case .monthly:  [.nanosecond, .second, .minute, .hour, .day]
                    case .yearly:   [.nanosecond, .second, .minute, .hour, .day, .month]
                }
#else
                let components: Calendar.ComponentSet = switch recurrence.frequency {
            ...
            mutating func nextGroup() {
                ...
                componentCombinations.hours   = if hourAction   == .expand { recurrence.hours   } else { nil }
                componentCombinations.minutes = if minuteAction == .expand { recurrence.minutes } else { nil }
                componentCombinations.seconds = if secondAction == .expand { recurrence.seconds } else { nil }
    ...
    internal func _expandOrLimitDaysOfTheMonth(dates: inout [Date], anchor: Date, action: ComponentAction) {
        if action == .limit {
            dates = dates.filter { date in
                ...
                return daysOfTheMonth.map {
                    if $0 > 0 {
                        $0
                    } else {
                        dayRange!.upperBound + $0
                    }
                }.contains(day)
            }
        } else {
            let components: Calendar.ComponentSet = [
                .day,
                .hour,
                .minute,
                .second,
            ...
            if frequency == .yearly {
                ...
                let monthsToDaysInMonth = monthRange.reduce(into: [Int: Int]()) {
                    dict, month in
    ...
    internal func _expandOrLimitDaysOfTheYear(dates: inout [Date], anchor: Date, action: ComponentAction) {
        if action == .limit {
            dates = dates.filter { date in
                ...
                let dayRange = calendar.range(of: .dayOfYear, in: .year, for: date)
                return daysOfTheYear.map {
                    if $0 > 0 {
                        $0
                    } else {
                        dayRange!.upperBound + $0
            ...
            let daysInYear = calendar.range(of: .dayOfYear, in: .year, for: anchor)!.upperBound - 1
